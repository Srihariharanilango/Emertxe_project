...
{
    if (strcmp(strstr(argv[2], "."), ".bmp") == 0)                      //checks whether the given argument is same as the required name of the file
    {
        decInfo->d_src_image_fname = argv[2];
    }
    else
        return e_failure;
    if (argv[3] != NULL)                                                //checks if argument is null 
...
{
    if (open_files_dec(decInfo) == e_success)               //checks if files can be opened
    {
        printf("Open files is a success\n");
        if (decode_magic_string(decInfo) == e_success)                      //checks if magic string is decoded
        {
            printf("Decoded magic string Successfully\n");
            if (decode_file_extn_size(strlen(".txt"), decInfo->fptr_d_src_image) == e_success)      //checks if file ext size is decoded
            {
                printf("Decoded file extension size Succesfully\n");
                if (decode_secret_file_extn(decInfo->d_extn_secret_file, decInfo) == e_success)     //checks if secret file ext decoded
                {
                    printf("Decoded Secret File Extension Succesfully\n");
                    if (decode_secret_file_size(decInfo->size_secret_file, decInfo) == e_success)    //checks if secret file size decoded
                    {
                        printf("Decoded secret file size Successfully\n");
                        if (decode_secret_file_data(decInfo) == e_success)                  //checks if secret file data decoded
                        {
                            printf("Decoded secret file data Succuessfully\n");
                        }
                        else
                        {
                            printf("Decoding of secret file data is a failure\n");          //prints Decoding of secret file data is a failure
                        }
                    }
                    else
                    {
                        printf("Decode of secret file size is a failure\n");     //prints Decode of secret file size is a failure and returns e_failure
                        return e_failure;
                    }
                }
                else
                {
                    printf("Decode of Secret file extension is a failure\n");      //prints Decode of Secret file extension is a failure and returns e_failure
                    return e_failure;
                }
            }
            else
            {
                printf("Decoded of file extension size is a failure\n");        //prints Decoded of file extension size is a failure and returns e_failure
                return e_failure;
            }
        }
        else
        {
            printf("Decoding of magic string is a failure\n");              //prints Decoding of magic string is a failure and returns e_failure
            return e_failure;
        }
    }
    else
    {
        printf("Open files is a failure\n");                    //prints Open files is a failure and returns e_failure
        return e_failure;
    }
    return e_success;                                           //returns e_success
}
