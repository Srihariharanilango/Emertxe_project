...
unsigned char clear_log(){
    if(clear_flag==0){
        ...
        clcd_print("NO LOG PRESENT",(0x80 + 0));
        _delay((unsigned long)((1000)*(20000000/4000.0)));
        clear_display();
        return 0x02;
    }
    ...
    if(i==4){
        ...
        if(strncmp(user_passwd,s_passwd,4)==0){
            ...
            _delay((unsigned long)((100)*(20000000/4000000.0)));
                ...
                clcd_putch(attempt_left+'0',(0xC0 + 0));
...
unsigned char change_passwd()
{
    ...
    while (1)
    {
        ...
        if (key == 0x37 && in < 4)
        {
            clcd_putch('*', (0xC0 + 4 + in));
            new_passwd[in++] = '1';
        }
        else if (key == 0x2F && in < 4)
        {
            clcd_putch('*', (0xC0 + 4 + in));
            new_passwd[in++] = '0';
        }
        ...
        if (in == 4)
        {
            ...
            else
            {
                clear_display();
                clcd_write(0x0C, 0);
                clcd_print("PASSWORD DO NOT", (0x80 + 0));
...
unsigned char menu_screen(unsigned char key, unsigned char reset_flag, unsigned char *control_flag) {
    ...
    static unsigned char short_press_done = 0;
    ...
    if (key == 0x37 || key == 0x2F) {
        ...
        if (key == prev_key) {
            ...
            if (menu_delay > 20) {
                if (key == 0x37) {
                    ...
                    switch (menu_pos) {
                        case 0: *control_flag = 0x66; break;
                        case 1: *control_flag = 0x12; break;
                        case 2: *control_flag = 0x13; break;
                        case 3: *control_flag = 0x14; break;
                        case 4: *control_flag = 0x15; break;
                    }
                } else if (key == 0x2F) {
                    longpress = 1;
                    clear_display();
                    clcd_print("LOGGING OUT", (0x80 + 2));
                    _delay((unsigned long)((2000)*(20000000/4000.0)));
                    clear_display();
                    return 0x33;
                }
        ...
        if (prev_key != 0x3F && menu_delay <= 20 && longpress == 0) {

            if (prev_key == 0x37 && menu_pos > 0) {
                menu_pos--;
                clear_display();
            } else if (prev_key == 0x2F && menu_pos < 4) {
                menu_pos++;
                clear_display();
            }
            short_press_done = 1;
...
void set_time(void)
{
    ...
    time_fields[0] = read_ds1307(0x02);
    time_fields[1] = read_ds1307(0x01);
    time_fields[2] = read_ds1307(0x00);
    clear_display();
    ...
    while (1)
    {
        ...
        for (unsigned char i = 0; i < 3; i++)
        {
            ...
            {
                clcd_putch(' ', (0xC0 + pos));
                clcd_putch(' ', (0xC0 + pos + 1));
            }
            else
            {
                clcd_putch(((time_fields[i] >> 4) & 0x0F) + '0', (0xC0 + pos));
                clcd_putch((time_fields[i] & 0x0F) + '0', (0xC0 + pos + 1));
            }

            if (i < 2)
                clcd_putch(':', (0xC0 + pos + 2));
        }
        ...
        if (key == 0x37)
        {
            ...
            if (delay > 10)
            {
                write_ds1307(0x02, time_fields[0]);
                write_ds1307(0x01, time_fields[1]);
                write_ds1307(0x00, time_fields[2]);

                clear_display();
                clcd_print("Time Set Success", (0x80 + 0));
                _delay((unsigned long)((2000)*(20000000/4000.0)));
                return;
            }
        }
        else if (key == 0x3F && old_key == 0x37)
        {
            if (delay > 0 && delay <= 10)
            {
...
unsigned char view_log()
{
        ...
        while (1)
        {
            ...
            while (1)
            {
                ...
                if (key == 0x37)
                {
                    ...
                    {
                        clear_display();
                        while (read_digital_keypad(0) != 0x3F);
                        return 0x02;
                    }
                }
    ...
    {
        clear_display();
        clcd_print("NO LOG FOUND", (0x80 + 1));
        _delay((unsigned long)((3000)*(20000000/4000.0)));
        clear_display();
        return 0x02;
    }
